...
static void test_missing_version_field_fails_open(void)
{
    Elf64_Rela plt_relas[] = {
        {
            .r_offset = 0x3010,
    ...
    check_int("missing-version.collect",
              kzt_runtime_got_plt_candidates_collect(&request, &result), 0);
    check_fail_open(
        "missing-version", &result,
        KZT_RUNTIME_GOT_PLT_CANDIDATE_REASON_MISSING_DYNAMIC_FIELD,
        KZT_PATCH_REASON_INPUT_MALFORMED_SYMBOL_VERSION);
    check_int("missing-version.table", result.table_kind,
              KZT_PATCH_TABLE_PLT_RELA);
}
...
int main(int argc, char **argv)
{
    ...
    if (test_matches_filter("missing_version_field_fails_open",
                            argc, argv)) {
        test_missing_version_field_fails_open();
    }

...
for required in (
    ...
    if required not in consumer:
        raise AssertionError(f"per-object flow misses scoped handoff: {required}")
if "kzt_tb_callback_scope_t callback_scope;" not in consumer:
...
write_call = per_object.find("kzt_per_object_got_plt_apply(&request, &result)")
if materialize_call < 0 or write_call < 0 or materialize_call >= write_call:
...
for required in (
    ...
    if required not in per_object:
        raise AssertionError(f"per-object write request misses {required}")
for required in (
...
for required in (
    ...
    if required not in writer:
        raise AssertionError(f"per-object writer misses {required}")
if "kzt_production_lazy_prebind_object(" in writer:
    raise AssertionError("per-object writer prebinds before guest PLT relocation")
relocate = function_body(elfloader, "int RelocateElfPlt(")
if "kzt_production_lazy_prebind_object(" in relocate or \
        "kzt_production_lazy_prebind_refresh(" in relocate:
    raise AssertionError("RelocateElfPlt prebinds before the guest loader event")
...
for required in (
    ...
    "production_lazy_prebind_publish_record(",
):
    if required not in prebind:
        raise AssertionError(f"per-object prebind misses guarded write step: {required}")

publish_record = function_body(
    production, "static int production_lazy_prebind_publish_record("
)
for required in (
    "kzt_lazy_prebind_scope_publish_acquire(scope, record, &lease)",
    "production_lazy_prebind_slot_cas(",
    "kzt_lazy_prebind_scope_publish_finish(&lease, committed)",
):
    if required not in publish_record:
        raise AssertionError(f"per-object slot publication misses {required}")

...
static int fake_read_slot(uintptr_t slot_addr, uintptr_t *value, void *opaque)
{
    ...
    ++slot->read_calls;
    if (slot->force_verify_mismatch && slot->read_calls == 2) {
...
static int fake_begin_write(uintptr_t slot_addr,
                            kzt_patch_spike_permission_lease_t *lease,
                            void *opaque)
{
    fake_slot_t *slot = opaque;

    if (!slot || !lease || slot_addr != slot->slot_addr) {
        return -1;
    }
    ++slot->begin_calls;
    lease->checked = 1;
    ...
    if (slot->fail_permission_begin) {
        return -1;
    }
    lease->write_enabled = 1;
    if (slot->fail_permission_begin_after_enable) {
        return -1;
    }
    return 0;
}

static int fake_end_write(kzt_patch_spike_permission_lease_t *lease,
                          void *opaque)
{
    fake_slot_t *slot = opaque;
...
static int fake_validate_generation(const kzt_patch_decision_t *decision,
                                    void *opaque)
{
    fake_slot_t *slot = opaque;

    return !decision || !slot || slot->fail_generation ? -1 : 0;
}
...
static kzt_patch_spike_slot_ops_t fake_slot_ops(fake_slot_t *slot)
{
    return (kzt_patch_spike_slot_ops_t) {
        ...
        .begin_write = fake_begin_write,
        .end_write = fake_end_write,
        .validate_generation = fake_validate_generation,
        .opaque = slot,
    };
}
...
static void test_verify_failure_rolls_back_successfully(void)
{
    ...
    check_int("verify-fail.read-calls", slot.read_calls, 3);
    ...
    check_int("verify-fail.rollback-verify",
...
static void test_permission_enable_failure_does_not_write(void)
{
    kzt_patch_decision_t decision = approved_decision(0x7100000018);
    kzt_patch_spike_guard_t guard = init_guard(1, 1, 8);
    kzt_patch_spike_record_t record;
    ...
    check_int("permission-enable.apply",
              kzt_patch_spike_writer_try_apply_with_slot_ops(
                  &guard, &decision, &ops, &record), 0);
    check_int("permission-enable.failure", record.failure,
              KZT_PATCH_SPIKE_FAILURE_PERMISSION_ENABLE_FAILED);
    check_int("permission-enable.begin", slot.begin_calls, 1);
    check_int("permission-enable.no-read", slot.read_calls, 0);
    check_int("permission-enable.no-write", slot.write_calls, 0);
    check_int("permission-enable.checked", record.permission_checked, 1);
    check_int("permission-enable.opened", record.permission_write_enabled, 0);
...
static void test_partial_permission_enable_is_restored(void)
{
    ...
    fake_slot_t slot = {
        ...
        .replacement_value = decision.bridge_target,
        .previous_value = decision.slot_current_value,
        .fail_permission_begin_after_enable = 1,
    };
    kzt_patch_spike_slot_ops_t ops = fake_slot_ops(&slot);

    check_int("permission-partial.apply",
              kzt_patch_spike_writer_try_apply_with_slot_ops(
                  &guard, &decision, &ops, &record), 0);
    check_int("permission-partial.failure", record.failure,
              KZT_PATCH_SPIKE_FAILURE_PERMISSION_ENABLE_FAILED);
    check_int("permission-partial.begin", slot.begin_calls, 1);
    check_int("permission-partial.end", slot.end_calls, 1);
    check_int("permission-partial.restore", record.permission_restored, 1);
    check_int("permission-partial.no-write", slot.write_calls, 0);
}

static void test_generation_mismatch_does_not_touch_permissions_or_slot(void)
{
    kzt_patch_decision_t decision = approved_decision(0x7100000018);
    kzt_patch_spike_guard_t guard = init_guard(1, 1, 8);
    kzt_patch_spike_record_t record;
    ...
    check_int("generation.apply", kzt_patch_spike_writer_try_apply_with_slot_ops(
                  &guard, &decision, &ops, &record), 0);
    check_int("generation.failure", record.failure,
              KZT_PATCH_SPIKE_FAILURE_GENERATION_MISMATCH);
    check_int("generation.result", record.result,
              KZT_PATCH_SPIKE_RESULT_GUEST_PRESERVED);
    check_int("generation.checked", record.generation_checked, 1);
    check_int("generation.match", record.generation_matched, 0);
    check_int("generation.no-permission", slot.begin_calls, 0);
    check_no_slot_calls("generation.slot", &slot);
}
...
static void test_record_fields_are_complete(void)
{
    ...
    decision.source.generation = 42;
    decision.current_owner.generation = 43;
    decision.dynamic_view_generation = 44;
    ...
    check_uintptr("record.source-link-map", record.source_link_map,
                  decision.source.link_map_addr);
    check_uintptr("record.owner-link-map", record.current_owner_link_map,
                  decision.current_owner.link_map_addr);
    check_ulong("record.source-generation", record.source_generation, 42);
    check_ulong("record.owner-generation", record.current_owner_generation,
                43);
    check_ulong("record.dynamic-generation", record.dynamic_view_generation,
                44);

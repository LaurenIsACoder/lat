...
static kzt_rela_immediate_candidate_request_t base_request(void)
{
    return (kzt_rela_immediate_candidate_request_t) {
        ...
        .expected_guest_target = 0x7100005678,
        .native_bridge_target = 0x7200004560,
        .legacy_target = 0x7300004560,
        .symbol_index = 33,
    ...
    const kzt_rela_immediate_candidate_request_t *request)
{
    return (wi231_fake_slot_t) {
        ...
        .replacement_value = request->native_bridge_target,
    };
}
...
static void test_target_fields_keep_separate_meanings(void)
{
    kzt_rela_immediate_candidate_request_t request = base_request();
    kzt_rela_immediate_candidate_result_t result;

    request.expected_guest_target = 0x7100001111;
    request.native_bridge_target = 0x7200002222;
    request.legacy_target = 0x7300003333;

    check_int("target_roles.call",
              kzt_rela_immediate_jump_slot_plan(&request, &result), 0);
    check_int("target_roles.decision", result.decision.kind,
              KZT_PATCH_DECISION_APPROVED);
    check_ulong("target_roles.bridge", result.candidate.bridge_target,
                request.native_bridge_target);
    check_ulong("target_roles.decision_bridge",
                result.decision.bridge_target,
                request.native_bridge_target);

    request.native_bridge_target = 0;
    check_int("target_roles.zero_bridge.call",
              kzt_rela_immediate_jump_slot_plan(&request, &result), 0);
    check_int("target_roles.zero_bridge.decision", result.decision.kind,
              KZT_PATCH_DECISION_UNSUPPORTED);
    check_int("target_roles.zero_bridge.reason", result.decision.reason,
              KZT_PATCH_REASON_INPUT_UNAVAILABLE_BRIDGE_TARGET);
    check_ulong("target_roles.zero_bridge.candidate",
                result.candidate.bridge_target, 0);
}

static void test_explicit_unversioned_and_unknown_evidence(void)
{
    kzt_rela_immediate_candidate_request_t request = base_request();
    kzt_rela_immediate_candidate_result_t result;
    ...
    request.wrapper_match = KZT_PATCH_WRAPPER_UNVERSIONED_MATCH;
    request.wrapper_version_evidence =
        KZT_SYMBOL_VERSION_CONFIRMED_UNVERSIONED;
    request.wrapper_symbol_version = NULL;
    check_int("unversioned-plan.call",
              kzt_rela_immediate_jump_slot_plan(&request, &result), 0);
    check_int("unversioned-plan.status", result.status,
              KZT_RELA_IMMEDIATE_CANDIDATE_PLANNED);
    check_int("unversioned-plan.approved", result.decision.kind,
              KZT_PATCH_DECISION_APPROVED);
    slot = wi231_slot_from_request(&request);
    wi231_apply_step4_request_contract(
        &request, &slot, request.legacy_target, &route);
    check_int("unversioned-write.applied", route.record.result,
              KZT_PATCH_SPIKE_RESULT_APPLIED);
    check_int("unversioned-write.skip-legacy", slot.legacy_write_calls, 0);
    check_ulong("unversioned-write.final", slot.value,
                request.native_bridge_target);

    request = base_request();
    request.version_evidence = KZT_SYMBOL_VERSION_UNKNOWN;
    check_int("unknown-plan.call",
              kzt_rela_immediate_jump_slot_plan(&request, &result), 0);
    check_int("unknown-plan.status", result.status,
              KZT_RELA_IMMEDIATE_CANDIDATE_FAIL_OPEN);
    check_int("unknown-plan.reason", result.reason,
    ...
    void)
{
    ...
    check_ulong("wi231.success.final", slot.value, request.native_bridge_target);
...
int main(void)
{
    ...
    test_target_fields_keep_separate_meanings();
    test_explicit_unversioned_and_unknown_evidence();

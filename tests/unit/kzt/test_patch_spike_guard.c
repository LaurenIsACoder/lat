...
static void test_rollback_failure_opens_circuit_breaker(void)
{
    ...
    check_int("rollback-failure.second-skip-legacy",
              outcome.skip_legacy_write, 1);
    check_int("rollback-failure.reads-after", writer.read_calls, 1);
    check_int("rollback-failure.writes-after", writer.write_calls, 1);
}

typedef struct concurrent_budget_case {
    kzt_patch_spike_guard_t *guard;
    kzt_patch_decision_t decision;
    kzt_patch_spike_writer_ops_t ops;
    kzt_patch_spike_outcome_t outcome;
} concurrent_budget_case_t;

static void *run_concurrent_budget_case(void *opaque)
{
    concurrent_budget_case_t *test = opaque;

    kzt_patch_spike_guard_try_write(test->guard, &test->decision,
                                    &test->ops, &test->outcome);
    return NULL;
}

static void test_concurrent_budget_reservation_allows_exactly_one_writer(void)
{
    enum { THREADS = 8 };
    kzt_patch_spike_guard_t guard = init_guard(1, 1, 1);
    fake_writer_t writer = {
        .current_value = 0x7200001000,
    };
    kzt_patch_spike_writer_ops_t ops = writer_ops(&writer);
    concurrent_budget_case_t cases[THREADS];
    pthread_t threads[THREADS];
    int i;
    int applied = 0;

    for (i = 0; i < THREADS; ++i) {
        cases[i] = (concurrent_budget_case_t) {
            .guard = &guard,
            .decision = approved_decision(),
            .ops = ops,
        };
        check_int("concurrent-budget.create",
                  pthread_create(&threads[i], NULL,
                                 run_concurrent_budget_case, &cases[i]), 0);
    }
    for (i = 0; i < THREADS; ++i) {
        check_int("concurrent-budget.join", pthread_join(threads[i], NULL),
                  0);
        applied += cases[i].outcome.result == KZT_PATCH_SPIKE_RESULT_APPLIED;
    }
    check_int("concurrent-budget.applied", applied, 1);
    check_int("concurrent-budget.writer-calls", writer.write_calls, 1);
    check_ulong("concurrent-budget.attempts", guard.write_attempts, 1);
    check_ulong("concurrent-budget.successes", guard.write_successes, 1);
    check_int("concurrent-budget.circuit",
...
int main(void)
{
    ...
    test_concurrent_budget_reservation_allows_exactly_one_writer();

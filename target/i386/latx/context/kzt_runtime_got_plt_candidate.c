    ...
    const char **out)
{
    ...
    if (!pool->storage || pool->used >= pool->size) {
        return -1;
    }
    start = pool->used;
    readable = pool->size - start;
    if (remaining < readable) {
        readable = (size_t)remaining;
    }
    if (request->reader_ops->read_memory(
            string_addr, &pool->storage[start], readable,
            request->reader_ops->opaque) != 0) {
        return -1;
    }
    terminator = memchr(&pool->storage[start], '\0', readable);
    if (!terminator || terminator == &pool->storage[start]) {
        return -1;
    }
    pool->used = (size_t)(terminator - pool->storage) + 1;
    ...
    const Elf64_Rela *rela)
{
    ...
    if (kzt_runtime_got_plt_read_symbol_version(
            request, result, pool, symbol_index, table_kind, entry_index,
            entry_addr, &version_evidence, &version_name) != 0) {
        return -1;
    }
    ...
    candidate.version_evidence = version_evidence;
    ...
    size_t *candidate_count)
{
    ...
    if (request->only_entry && request->only_table_kind != table_kind) {
        return 0;
    }
    ...
    if (request->only_entry) {
        if (request->only_entry_index >= entry_count) {
            kzt_runtime_got_plt_fail_open(
                result, KZT_RUNTIME_GOT_PLT_CANDIDATE_REASON_MALFORMED_TABLE,
                KZT_PATCH_REASON_INPUT_MALFORMED_TABLE, table_kind,
                request->only_entry_index, table_addr, 0, 0);
            return -1;
        }
        i = request->only_entry_index;
    } else {
        i = 0;
    }
    for (; i < entry_count; ++i) {
    ...
    size_t *candidate_count)
{
    ...
    if (request->only_entry &&
        request->only_table_kind != KZT_PATCH_TABLE_RELA) {
        return 0;
    }
    ...
    kzt_runtime_got_plt_candidate_result_t *result)
{
    ...
    if ((!request->only_entry ||
         request->only_table_kind == KZT_PATCH_TABLE_REL) &&
        kzt_runtime_got_plt_has_rel_table(request->view)) {
        return kzt_runtime_got_plt_fail_open(
            result, KZT_RUNTIME_GOT_PLT_CANDIDATE_REASON_DT_REL_UNSUPPORTED,
            KZT_PATCH_REASON_INPUT_UNSUPPORTED_RELOCATION,

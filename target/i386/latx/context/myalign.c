...
static void kzt_install_guest_fork_callback(CPUState *cpu);
...
static uintptr_t kzt_guest_fork_addr;
extern void* x86free;
    ...
    if (result == KZT_PUBLIC_LOADER_OK) {
        for (size_t external_index = 0;
            ...
            tls_object.load_generation =
                kzt_guest_tls_external_maps[external_index]
                    .load_generation;
            tls_object.external_registration = 1;
...
static void kzt_dynamic_library_change_callback(CPUX86State *env)
{
    ...
    if (latx_kzt_guest_tls_enabled() &&
        (result == KZT_PUBLIC_LOADER_OK ||
         result == KZT_PUBLIC_LOADER_LIMIT)) {
        int event_result = kzt_guest_tls_loader_event_observe();

        if (event_result < 0) {
            fprintf(stderr,
                    "KZT cannot observe Guest TLS loader generation; "
...
static void kzt_guest_main_entry_callback(CPUX86State *env)
{
    ...
    if (observer_result == KZT_PUBLIC_LOADER_OK) {
        ...
        env->eip = eip;
        kzt_install_guest_fork_callback(cpu);
        printf_log(LOG_INFO,
...
static void kzt_install_guest_fork_callback(CPUState *cpu)
{
    ...
    if (!kzt_guest_fork_addr) {
        /* libpthread and libc can both export fork at different addresses. */
        kzt_guest_fork_addr = kzt_resolve_guest_object_symbol("libc.so.6", "fork");
        if (!kzt_guest_fork_addr) {
            kzt_guest_fork_addr = kzt_resolve_guest_symbol("__libc_fork");
        }
    }
    if (kzt_guest_fork_addr) {
        /* A full flush invalidates the saved translated instructions too. */
        memset(kzt_guest_fork_inst, 0, sizeof(kzt_guest_fork_inst));
        kzt_install_guest_pc_callback(kzt_guest_fork_inst, cpu,
            kzt_guest_fork_addr, kzt_guest_fork_entry_callback);
    }
    env->eip = eip;
}
